...
PBS における task とは、**特定の担当者が、一定期間内に、明確な行動として実行・判断・確認・返信・調整・作成すべき仕事の最小管理単位**を指す。
...
ただし、判断・確認・提案準備そのものが実務として必要な場合は task 化してよい。

最終定義:

- `task`: 行動あり / 担当あり / 放置コストあり / 完了判定あり
- `review`: 重要そうだが task 断定は弱い
- `drop`: 情報・報告・雑談・完了済み・他人専用
