...
- Minutes:
  ...
  - `duplicate_same_doc_action`
  - `too_broad`
  - `not_actionable`
    の件数を重点的に削減する

### 2.5.1 Minutes task quality gate

議事録・Google Meet docs 由来 task の品質は、単なる task 抽出精度ではなく、Neuronic に入った後の使いやすさまで含めて評価する。

- project evidence:
  - seed / doc / section / keyword hint / registry のいずれかで project 根拠があること
  - 根拠が弱い場合は task 投入ではなく review に寄せる
- semantic parent:
  - `parent_origin_id` が存在するだけでは合格としない
  - child title/note の強い固有語が別 project を示す場合は `semantic_parent_misnested` として扱う
- duplicate:
  - 同一 doc・同一 project・類似 action は原則1件に統合する
  - auto parent の重複と child action の重複は別 failure mode として見る
- stable identity:
  - `origin_id` は抽出順や group index に依存させない
  - parent/child は同じ send group として cap / retry / create_only に通す

### 2.6 Gemini 初回投入フェーズ

- 現在の Precision Sprint は、local-first の前処理基盤が揃い、`pbs_context_seed.md` も実処理へ接続済みのため、Gemini 初回投入へ進める状態にある
- ここでの「初回投入」は、毎回の本番 task 判定ではなく、**仕事理解と曖昧ケース判定の土台を作るための初期知識化** を指す
- 方針:
  - local 側では「明確なノイズ除去」までに留める
  - 曖昧な候補は Gemini に渡して `task / review_only / drop / rewrite` を判断させる
  - 大量投入は初期構築フェーズ中心とし、定常運用では少量・限定投入へ切り替える

#### 初回投入の対象

- `/Users/shu/OpenClaw/docs/pbs_context_seed.md`
- TOKIWAGI_MASTER の議事録 corpus
- `基礎情報`
- 代表 GDocs / Notion 議事録
- good / bad / missed の代表例
- local 前処理で得た以下の構造化情報
  - section ごとの project
  - owner hints
  - action candidates
  - task にしやすい / しにくい基準
...
- project ごとの背景理解と固有語の整理
- owner 境界の理解
- 「task にしてよいメモ / review に留めるメモ / drop してよいメモ」の判断軸整理
- 曖昧な action candidate の最終判定
- task 名の rewrite
- parent / subtask の自然な分解

#### 初回投入フェーズでやらないこと

- すべての過去メール本文を丸ごと毎回 Gemini に投入すること
- 全議事録を毎回 task 化すること
- local ルールで candidate を締め切って Gemini へ渡す材料まで消すこと

#### 初回投入の実行順

1. corpus と seed の対象一覧を確定する
2. `scripts/roby-gemini-budget.py` で token / cost 見積りを出す
3. `confirm_required` 以上ならユーザー確認を取る
4. Gemini 初回投入を実行する
5. 出力を `docs/pbs_gemini_initial_seed_review.md` で採用 / 保留 / 不採用に仕分ける
6. 採用した判断軸だけを registry / adjudication ルール / prompt 改善へ反映する
7. representative case と precision eval で改善結果を確認する

#### 初回投入の実行済みメモ（2026-03-16）

- 実行モデル: `google/gemini-3.1-pro-preview`
- 実投入 bundle: seed-only 一式 + 代表 Notion 10本 + 代表 GDocs 2本
- 実出力: `/Users/shu/.openclaw/roby/pbs_gemini_initial_seed_result_20260316.md`
- レビュー整理: `/Users/shu/OpenClaw/docs/pbs_gemini_initial_seed_review.md`
- 運用方針:
  - Gemini の返答をそのまま仕様化しない
  - `pbs_context_seed.md` と最近の human-confirmed ルールを正本として優先する
  - Gemini 返答は「判断軸の補助資料」として採用 / 保留 / 不採用に分けて使う
...
- local で
  ...
  - action candidate
    まで出す
- Gemini は、曖昧候補の adjudication と最終整形だけに使う
- 再度の大量投入は、案件構造が大きく変わった時や、registry 再構築が必要な時だけ例外的に行う
...
- 低信頼なものは `review` に降格し、run log も降格後バケットを使う（Gmail triage の Slack通知は廃止）
...
- adjudication が `0件` になった場合は、date / action signal を持つ high-confidence 候補を 1〜2件だけ rescue する
- これにより
