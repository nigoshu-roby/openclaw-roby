...
- Gmail の文章理解から task extraction までは Gemini API を標準に変更。
  ...
  - 実メール `Re: 【ボーネルンド】夏のあそび場販促準備に伴う確認のご依頼` で、waiting follow-up として `再依頼が来たらベンダーに依頼内容を共有する` / `本日中に再依頼がなければクライアントに確認する` を抽出。
  - 実メール `Re: 予約システムのヒアリングについて` で、waiting follow-up として `ダブルスタンダード社のプロバイダID確認状況を確認する` / `確認結果に応じてベンダーへ相談する` を抽出。
...
| A2  | 4分類（archive/digest/review/task）導入   | 完了 | #13          | 運用バケット導入済み。Gmail triage の Slack通知は廃止し、Neuronic投入は task のみ              |
