...
class MorningCommandTests(unittest.TestCase):
    ...
    def test_build_payload_keeps_generic_email_and_finance_out_of_focus(self):
        tasks = [
            {
                "origin_id": "project",
                "title": "BW本番環境移行の判断を進める",
                "status": "inbox",
                "source": "roby",
                "project": "LINE広告配信",
                "updated_at": "2026-07-07T08:00:00+09:00",
            },
            {
                "origin_id": "generic",
                "title": "【s.nigo@tokiwa-gi.com】依頼内容を確認して対応する",
                "status": "inbox",
                "source": "roby",
                "project": "email",
                "updated_at": "2026-07-07T08:30:00+09:00",
            },
            {
                "origin_id": "finance",
                ...
                "source": "roby",
                "project": "請求・経理",
                "updated_at": "2026-07-07T08:45:00+09:00",
            },
        ]
        ...
        self.assertEqual([row["origin_id"] for row in payload["focus"]], ["project"])
        self.assertEqual({row["origin_id"] for row in payload["admin_review"]}, {"generic", "finance"})

    def test_build_payload_respects_focus_limit(self):
        tasks = [
            {
                "origin_id": f"o{i}",
                "title": f"ボーネルンドの判断事項 {i} を確認する",
                "status": "inbox",
                "source": "roby",
                "project": "ボーネルンド",
                "updated_at": "2026-07-07T08:00:00+09:00",
        ...
        self.assertEqual(len(payload["focus"]), 2)
