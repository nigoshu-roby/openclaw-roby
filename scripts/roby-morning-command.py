...
def task_score(row: Dict[str, Any], today: datetime) -> int:
    ...
    if str(get_any(row, "status") or "").lower() == "inbox":
        score += 1
    if infer_project(row) == "請求・経理" and due_score(row, today) == 0:
        score -= 3
...
def select_focus_tasks(open_tasks: List[Dict[str, Any]], today: datetime, limit: int) -> List[Dict[str, Any]]:
    ...
    for row in ordered:
        project = infer_project(row)
        cap = 2 if project in {"email", "請求・経理"} else 3
...
def build_goals(project_health: List[Dict[str, Any]], focus: List[Dict[str, Any]], limit: int = 3) -> List[str]:
    ...
    for row in project_health:
        if row["state"] not in {"red", "yellow"}:
            continue
        project = row["project"]
...
def build_payload(tasks: List[Dict[str, Any]], *, base_url: str = "", generated_at: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    focus = select_focus_tasks(open_tasks, today, 8)
    ...
    return {
        ...
        "next_prompt": "今日のゴールを確認し、Focusを3件以内に絞ってから着手してください。",
    }
...
def main() -> int:
    ...
    payload = build_payload(tasks, base_url=base_url)
