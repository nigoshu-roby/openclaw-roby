...
export async function backupVerifyCommand(
  ...
  const normalizedEntrySet = new Set<string>();
  for (const entry of entries) {
    if (isRootManifestEntry(entry.normalized)) {
      normalizedEntrySet.add(entry.normalized);
      continue;
    }
    if (normalizedEntrySet.has(entry.normalized)) {
      throw new Error(`Archive contains duplicate entry path: ${entry.normalized}`);
    }
    normalizedEntrySet.add(entry.normalized);
  }
